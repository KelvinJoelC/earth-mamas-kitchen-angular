...
import { CheckoutDetails, CheckoutStep } from '../models/checkout-details.model';

@Injectable({ providedIn: 'root' })
export class CheckoutStore {

  private readonly _step = signal<CheckoutStep>('details');
  private readonly _details = signal<CheckoutDetails | null>(null);

  readonly step = computed(() => this._step());
  readonly details = computed(() => this._details());

  readonly isDetailsCompleted = computed(() =>
    !!this._details()?.email && !!this._details()?.fullName
  );
  ...
  setDetails(details: CheckoutDetails) {
